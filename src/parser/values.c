...
wasm_parser_error_t wasm_parse_name(wasm_parser_t *parser, size_t start,
  ...
  uint32_t length = 0;
  wasm_parser_error_t error = wasm_parse_uint(32, parser, 0, &length, end);
  if (error != WASM_PARSER_NO_ERROR)
    return error;

  if (length < 1)
    return WASM_PARSER_INVALID_NAME_LENGTH;

  wasm_utf8_t *_result = (wasm_utf8_t *)calloc(length + 1, sizeof(wasm_utf8_t));
  if (_result == NULL)
    return WASM_PARSER_CALLOC_FAILED;

  for (size_t i = 0; i < length; i++) {
    if (start + 1 + i >= parser->length)
      return WASM_PARSER_REACHED_END;
